...
| Stable	| Contributed	| Formal Review (light)	| The author [requests a formal peer review](https://docs.civicrm.org/dev/en/latest/extensions/publish/#automated-distribution).  Once the reviewer is satisfied, they mark the node in `civicrm.org` as Stable.	| In app, go to "Add New" and choose the extension.
